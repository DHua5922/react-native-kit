...
const exampleWithStateCode = `function PaginatedList() {
  ...
  return (
    <>
      <Table border hover striped>
        <Table.tr>
          {headers.map((header, index) => <Table.th key={index}>{header}</Table.th>)}
        </Table.tr>
      ...
      <Center>
        <Pagination style={{ marginTop: 20 }}>
            ...
            <Pagination.Item onPress={() => setCurrentPage(Math.max(0, currentPage - 1))}>
                <Pagination.Previous />
            </Pagination.Item>
                
            <Pagination.Item disabled>
                <Text>Page {currentPage + 1} of {Math.ceil(list.length / pageSize)}</Text>
            </Pagination.Item>
                
            <Pagination.Item onPress={() => setCurrentPage(Math.min(currentPage + 1, Math.floor(list.length / pageSize)))}>
                <Pagination.Next />
            </Pagination.Item>

            <Pagination.Item onPress={() => setCurrentPage(Math.floor(list.length / pageSize))}>
                <Pagination.Last />
            </Pagination.Item>
        </Pagination>
      </Center>
...
const exampleWithStateAndFilterCode = `
function PaginatedList() {
  ...
  const [tableHeaders, setTableHeaders] = useState(headers.map(header => ({
      ...header,
      showMenu: false,
  ...
  function modifyTableHeader(updatedTableHeader, index) {
    function differentChosenSortMapping() {
      const hasChoseDifferentSortProperty = updatedTableHeader.isSortAscending || updatedTableHeader.isSortDescending;
      return (header) => ({
        ...header,
        isSortAscending: hasChoseDifferentSortProperty ? false : header.isSortAscending,
        isSortDescending: hasChoseDifferentSortProperty ? false : header.isSortDescending
      })
    }
  ...
  function filterList() {
    ...
    const headerWithSort = tableHeaders.find(header => header.isSortAscending || header.isSortDescending);
    headerWithSort && newList.sort((personA, personB) => headerWithSort.isSortAscending
      ? sort(personA[headerWithSort.propertyName], personB[headerWithSort.propertyName])
      : sort(personB[headerWithSort.propertyName], personA[headerWithSort.propertyName]));
    ...
    <>
              ...
              <Menu.Toggle
                Toggle={(
                  <Row>
                    ...
                    {(header.isCheckingEqual || header.isCheckingContains) && <FontAwesome5 name="filter" size={20} />}
                    {header.isSortAscending && "↑"}
                    {header.isSortDescending && "↓"}
                  </Row>
                )}
                showMenu={header.showMenu}
                onShowMenu={() => modifyTableHeader({ showMenu: true }, columnIndex)}
                onHideMenu={() => modifyTableHeader({ showMenu: false }, columnIndex)}
              >
                  {[
                    {
                      label: "Clear Filter",
                      active: false,
                      onPress: () => modifyTableHeader({
                          ...header,
                          showMenu: false,
                          isSortAscending: false,
                          isSortDescending: false,
                  ...
                  <Menu.Item onPress={() => {}}>
                    <Row style={{ justifyContent: "flex-end" }}>
                      <Button
                        style={{ marginRight: 8 }}
                        onPress={() => {
                          setFilteredList(filterList(header.isSortAscending, header.isSortDescending, header.propertyName));
                          modifyTableHeader({
      ...
      <Center>
        <Pagination style={{ marginTop: 20 }}>
            ...
            <Pagination.Item disabled>
                <Text>Page {currentPage + 1} of {lastPageNumDisplay}</Text>
            </Pagination.Item>
                
            <Pagination.Item onPress={() => setCurrentPage(Math.min(currentPage + 1, lastPageIndex))}>
                <Pagination.Next />
            </Pagination.Item>
...
function PaginationPage(props: any) {
  return (
    <Page.Documentation {...props}>
      <Documentation.Section title={pageNames.Pagination}>
      ...
      <Documentation.Section subtitle="Basic Example">
        <Code.Editor scope={{ Pagination, Input }}>
          {basicExampleCode}
        </Code.Editor>
      </Documentation.Section>

      <Documentation.Section subtitle="Paginated Table Example">
        <Code.Editor
          scope={{ Pagination, Input, useState, Table, Center, Text }}
        >
          {exampleWithStateCode}
        </Code.Editor>
      </Documentation.Section>

      <Documentation.Section subtitle="Paginated Table With Filtering Example">
        <Code.Editor
          scope={{
            Pagination,
            Input,
